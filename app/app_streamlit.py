...
from buscador import buscar_en_docs   # ✅ ahora usamos el buscador que trabaja con PDFs

# Configuración de la página
st.set_page_config(page_title="Agente IA AntiumGear", layout="wide")

# Layout: logo a la izquierda, chat a la derecha
...
with col1:
    ...
    if os.path.exists(fondo_path):
        ...
        st.warning("⚠️ No se encontró la imagen del logo con fondo gamer.")

with col2:
    st.markdown("## 🤖 Chat AntiumGear")
    st.info("Estás conversando con un agente de IA que responde usando documentos oficiales de AntiumGear (PDFs).")
    ...
    if pregunta:
        if pregunta.lower() in ["hola", "buenas", "hi"]:
            ...
            # ✅ Buscar directamente en los PDFs oficiales
            respuesta, fuentes = buscar_en_docs(pregunta)
